...
(un-normalised) features in a single pass, producing:
  - A 4-panel figure per run (elevation profile, features, posteriors, amplitude)
  - A side-by-side summary comparison figure
...
Features
--------
  f_amp   : Butterworth-filtered amplitude level
  f_dA    : amplitude gradient dA/dx  (NaN artifact regions masked)
  f_dfree : flotation residual  Δfree = h_surf − H·(1 − ρ_ice/ρ_sw)
  f_dsurf : ice surface slope   d(h_surf)/dx

Normalised run
  Each feature normalised to zero mean, unit std in the floating reference
  section (x < FLOAT_FRAC × gz_lo). Gives equal voice to all features.

Raw run
  Features used at their physical scales. Whichever feature has the largest
  within-window variance dominates the IFM/FullCov likelihoods. Typically
  Δfree or amplitude level, depending on glacier type and season.

Models per run
  - 1D Gaussian : Δfree alone  (simplest, most interpretable)
  - IFM         : 4 features, independent channels (Xuan et al. 2007)
  - FullCov     : 4 features, full sample covariance (Xuan et al. 2007)
  Combined posterior = geometric mean (equal weights, log-space average).

Physics constraints
  - Search window:  [search_lo_km, gz_lo_km]  (must be downflow of InSAR GZ)
  - For floating→grounded transects (Petermann style): x=0 bright, x↑ dark
  - For grounded→floating→grounded (Helheim PINNED_GZ): supply the dark
    block as the reference section via --ref_lo / --ref_hi

Difference between runs
  When normalised and raw MAPs agree   → robust detection
  When they disagree by > ~3 km       → flag for manual review;
    gap indicates which physical signal dominates the transition
...
-----
  ...
  # Helheim float (PINNED_GZ, grounded reference block 47-57 km)
  python bocpd_grounding_point_v2.py \\
      --power   helheim_bed_power.csv \\
      --bed     helheim_bottom.csv \\
      --surface helheim_surface.csv \\
      --search_lo 47 --search_hi 61 \\
...
def plot_combined(d, results, label, gz_lo, gz_hi,
    ...
    fig = plt.figure(figsize=(16, 22), facecolor=C["bg"])
    gs  = gridspec.GridSpec(5, 1, height_ratios=[2.6, 1.4, 1.4, 1.4, 1.3],
    ...
    # ── Panels C & D: one subpanel per run, all 3 models + combined ────────
    model_specs = [
        ("cp_gauss", C["gauss"], "Gaussian"),
        ("cp_ifm",   C["ifm"],   "IFM"),
        ("cp_fcov",  C["fcov"],  "FullCov"),
    ]

    for panel_idx, (run_label, run_col, run_ls, run_ci, run_ps) in enumerate([
        ("Normalised", C["norm"], "-",  ci_n, r_n["posteriors"]),
        ("Raw",        C["raw"],  "--", ci_r, r_r["posteriors"]),
    ]):
        ax = axs[2 + panel_idx]; shade(ax); ref(ax)
        ax.axvspan(search_lo, search_hi if search_hi else gz_lo,
                   color="#e8f4e8", alpha=0.40, zorder=0)

        cp_comb = run_ps["cp_comb"]
        # Scale so the tallest of the 4 curves fills the panel
        sc = max(cp_comb.max(),
                 max(run_ps[k].max() for k, _, _ in model_specs))

        # Individual model posteriors with MAP vlines and labels
        for key, col, name in model_specs:
            cp   = run_ps[key]
            m_km = float(x_cp[np.argmax(cp)])
            ax.plot(x_cp, cp / sc, color=col, lw=1.6, ls=":",
                    alpha=0.80, zorder=4,
                    label=f"{name}  {m_km:.1f} km")
            ax.axvline(m_km, color=col, lw=1.1, ls=":", alpha=0.55, zorder=4)

        # Combined posterior (heavy)
        ax.fill_between(x_cp, 0, cp_comb / sc,
                        color=run_col, alpha=0.18, zorder=1)
        ax.plot(x_cp, cp_comb / sc, color=run_col, lw=2.6, ls=run_ls,
                zorder=6,
                label=f"Combined  {run_ci['MAP']:.1f} km")
        ax.axvline(run_ci["MAP"], color=run_col, lw=2.0, ls=run_ls, zorder=7)

        # 68% CI bracket
        yci = 0.06
        ax.annotate("", xy=(run_ci["lo68"], yci),
                    xytext=(run_ci["hi68"], yci),
                    arrowprops=dict(arrowstyle="<->", color=run_col, lw=1.4))
        ax.text((run_ci["lo68"] + run_ci["hi68"]) / 2, yci + 0.05,
                f"68% CI  [{run_ci['lo68']:.0f}–{run_ci['hi68']:.0f} km]",
                ha="center", fontsize=6.5, color=run_col)

        ax.set_ylabel(f"P(GP=x)\n{run_label}", fontsize=9.5, color=run_col)
        ax.tick_params(labelbottom=False, colors=C["text"], labelsize=8.5)
